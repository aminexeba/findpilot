...
import styles from './insurancefm.module.css';

export default function PublishInsurance() {
  ...
  // Fetch existing tags AND unique countries from the 'insurance' collection on mount
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 1. Upload image to Firebase Storage under the "insurance" folder
      const uniqueFileName = `${Date.now()}_${imageFile.name}`;
      const storageRef = ref(storage, `insurance/${uniqueFileName}`);

      const snapshot = await uploadBytes(storageRef, imageFile);
      const downloadURL = await getDownloadURL(snapshot.ref);

      // 2. Prepare Firestore Data for "insurance" collection
      ...
      await setDoc(doc(db, 'insurance', docId), {
        ...formData,
        tag: cleanTag,
        image: downloadURL, // Firebase Storage URL
        slug: `/${slug}`,
      ...
      setMessage('Insurance listing published successfully with image!');
      ...
      const fileInput = document.getElementById('insuranceImageFileInput');
  ...
  return (
    <div className={styles.container}>
      <div className={styles.card}>
        <h2 className={styles.heading}>Publish Insurance Listing</h2>
        {message && <p className={styles.message}>{message}</p>}

        <form onSubmit={handleSubmit} className={styles.form}>
          <div className={styles.grid}>
            <div className={styles.field}>
              <label>Brand (Doc ID)</label>
              <input
                type="text"
                name="brand"
            ...
            <div className={styles.field}>
              ...
              <input
                ...
                onChange={handleChange}
                required
                placeholder="e.g., Nomad Travel Insurance"
              />
            </div>
            ...
            <div className={styles.field}>
              ...
              <input
                type="text"
                name="tag"
                list="insurance-tags-list"
                value={formData.tag}
                onChange={handleChange}
                required
                placeholder="Select existing tag or type a new one..."
              />
              <datalist id="insurance-tags-list">
            ...
            <div className={styles.field}>
              ...
              <input
                type="text"
                name="countries"
                list="insurance-countries-list"
                value={formData.countries}
                onChange={handleChange}
                required
                placeholder="Select existing or type comma-separated (e.g. US, UK)"
              />
              <datalist id="insurance-countries-list">
          ...
          <div className={styles.field}>
            ...
            <textarea
              ...
              rows={3}
              required
              placeholder="Key policy details or quick specifications..."
            />
          </div>

          <button type="submit" disabled={loading} className={styles.submitBtn}>
            {loading ? 'Uploading & Publishing...' : 'Publish Insurance'}
          </button>
        </form>
      </div>
    </div>
