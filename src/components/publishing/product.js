...
import styles from './productfm.module.css';
...
export default function PublishProduct() {
  ...
  // Fetch existing tags AND unique countries from the 'product' collection on mount
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 1. Upload image to Firebase Storage under the "product" folder
      const uniqueFileName = `${Date.now()}_${imageFile.name}`;
      const storageRef = ref(storage, `product/${uniqueFileName}`);
      ...
      setMessage('Product listing published successfully with image!');
  ...
  return (
    <div className={styles.container}>
      <div className={styles.card}>
        <h2 className={styles.heading}>Publish Product Listing</h2>
          ...
          <div className={styles.grid}>
            ...
            <div className={styles.field}>
              ...
              <input
                type="text"
                name="tag"
                list="product-tags-list"
                value={formData.tag}
                onChange={handleChange}
                required
                placeholder="Select existing tag or type a new one..."
              />
              <datalist id="product-tags-list">
