...
import styles from './servicefm.module.css';
...
export default function PublishService() {
  ...
  // Fetch existing tags AND unique countries from the 'service' collection on mount
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await setDoc(doc(db, 'service', docId), {
        ...formData,
        tag: cleanTag,
      ...
      setMessage('Service listing published successfully with image!');
  ...
  return (
    <div className={styles.container}>
      <div className={styles.card}>
        <h2 className={styles.heading}>Publish Service Listing</h2>
        ...
        <form onSubmit={handleSubmit} className={styles.form}>
          ...
          <div className={styles.field}>
            ...
            <textarea
              ...
              rows={3}
              required
              placeholder="Key service features or quick specifications..."
            />
          </div>

          <button type="submit" disabled={loading} className={styles.submitBtn}>
            {loading ? 'Uploading & Publishing...' : 'Publish Service'}
          </button>
        </form>
      </div>
    </div>
