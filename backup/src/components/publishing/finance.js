...
import styles from './financefm.module.css';

export default function PublishFinance() {
  ...
  // Fetch existing tags AND unique countries from the 'finance' collection on mount
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 1. Upload image to Firebase Storage under the "service" folder
      const uniqueFileName = `${Date.now()}_${imageFile.name}`;
      const storageRef = ref(storage, `service/${uniqueFileName}`);
      ...
      await setDoc(doc(db, 'finance', docId), {
        ...formData,
        tag: cleanTag,
        image: downloadURL,
        slug: `/${slug}`,
      ...
      setMessage('Finance listing published successfully with image!');
  ...
  return (
    <div className={styles.container}>
      <div className={styles.card}>
        <h2 className={styles.heading}>Publish Finance Listing</h2>
        ...
        <form onSubmit={handleSubmit} className={styles.form}>
          <div className={styles.grid}>
            ...
            <div className={styles.field}>
              ...
              <input
                ...
                onChange={handleChange}
                required
                placeholder="e.g., Revolut Business Account"
              />
            </div>
            ...
            <div className={styles.field}>
              ...
              <input
                type="text"
                name="tag"
                list="finance-tags-list"
                value={formData.tag}
                onChange={handleChange}
                required
                placeholder="Select existing tag or type a new one..."
              />
              <datalist id="finance-tags-list">
          ...
          <div className={styles.field}>
            ...
            <textarea
              ...
              rows={3}
              required
              placeholder="Key highlights or quick specifications..."
            />
          </div>
