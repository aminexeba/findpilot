...
import styles from './hotelfm.module.css';

export default function PublishHotel() {
  ...
  // Fetch existing tags AND unique countries from the 'hotel' collection on mount
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 1. Upload image to Firebase Storage under the "hotel" folder
      const uniqueFileName = `${Date.now()}_${imageFile.name}`;
      const storageRef = ref(storage, `hotel/${uniqueFileName}`);

      const snapshot = await uploadBytes(storageRef, imageFile);
      const downloadURL = await getDownloadURL(snapshot.ref);

      // 2. Prepare Firestore Data for "hotel" collection
      ...
      await setDoc(doc(db, 'hotel', docId), {
        ...formData,
        tag: cleanTag,
      ...
      setMessage('Hotel listing published successfully with image!');
  ...
  return (
    <div className={styles.container}>
      <div className={styles.card}>
        <h2 className={styles.heading}>Publish Hotel Listing</h2>
        ...
        <form onSubmit={handleSubmit} className={styles.form}>
          <div className={styles.grid}>
            ...
            <div className={styles.field}>
              ...
              <input
                ...
                onChange={handleChange}
                required
                placeholder="Select existing or type comma-separated (e.g. US, France)"
              />
          ...
          <div className={styles.field}>
            ...
            <textarea
              ...
              rows={3}
              required
              placeholder="Key hotel amenities or quick overview..."
            />
          </div>

          <button type="submit" disabled={loading} className={styles.submitBtn}>
            {loading ? 'Uploading & Publishing...' : 'Publish Hotel'}
          </button>
        </form>
      </div>
    </div>
